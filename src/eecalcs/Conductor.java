...
public class Conductor {
	...
	public static String[] sizes = {"14", "12", "10", "8", "6", "4", "3", "2", "1", "1/0", "2/0", "3/0", "4/0", "250", "300", "350",
	...
	private static String[] insulation75Celsius;//RHW, THW, THWN, USE, ZW. XHHW & THHW are duplicated in 75 and 90 degrees columns.
	// It is assumed both are 90 by definition of their double Hs
	private static String[] insulation90Celsius;//TBS, SA, SIS, FEP, FEPB, MI, RHH, RHW-2, THHN, THHW, THW-2, THWN-2, USE-2, XHH, XHHW,
	// XHHW-2, ZW-2};
	private static Message ERROR50	= new Message("\"%s\" is not a valid conductor size.", -50);
	private static Message ERROR51	= new Message("\"%s\" is not a valid insulation type.", -51);
	private static Message ERROR52	= new Message("Area of insulated conductor not available.", -52);
...
/*	private static String[] group1 = {"TW", "THHW", "THW", "THW-2"};
	...
	//todo refactor the name getAreaIn2 to getInsulatedAreaIn2
	public double getAreaIn2(String conductorSize, String insulationName){
...
/*		if(!dimensions.get(conduitType).containsKey(tradeSize)){
		...
		if(!isValidSize(conductorSize)) {
			resultMessages.add(conductorSize, ERROR50);
			return 0;
		}
//			throw new EEToolsException("\"" + conductorSize + "\"" + " is not a valid conductor size.");

		if(!isValidInsulationName(insulationName)) {
			resultMessages.add(ERROR51);
			return 0;
		}
//		throw new EEToolsException("\"" + insulationName + "\"" + " is not a valid insulation type.");

/*		if(dimensions.get(insulationName) == null){
			resultMessages.add(ERROR53);
			return 0;
		}*/

		if(!dimensions.get(insulationName).containsKey(conductorSize)){
			resultMessages.add(ERROR52);
			return 0;
		}
...
/*		int conductorSizeIndex = getIndexOfSize(conductorSize);
	...
	public int getInsulationTemperatureCelsius(String insulationName){
...
//		else throw new EEToolsException("\"" + insulationName + "\"" + " in not a valid insulation type.");
		resultMessages.add(insulationName, ERROR51);
		return 0;
	}
	...
	public String getSizeFullName(String size) {
		int index = getArrayIndexOf(Conductor.sizes, size);
		if(index < 0) {// throw new EEToolsException("Can't obtain a full conductor size name for an invalid conductor size: " + size);
			resultMessages.add(size, ERROR50);
			return "";
		}
